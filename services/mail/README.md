...
created. Address domain comes from `STALWART_DEFAULT_DOMAIN` (or `MAIL_DOMAIN`
with the `mail.` prefix stripped), e.g. `noreply@heylomeet.com`.

After creating the real SMTP account in Stalwart, prefer the host wrapper:

```sh
# From the Proxmox host / homelab repo:
...
Or update and restart the app inside the mail LXC:
