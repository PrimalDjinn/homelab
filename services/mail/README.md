...
The upstream email-service Stalwart generator currently writes the Stalwart
`v0.15` style `/opt/stalwart/etc/config.toml`, so the homelab defaults to
`STALWART_IMAGE=stalwartlabs/stalwart:v0.15`. Current Stalwart `v0.16` images
use a different `config.json` bootstrap/API model and will not consume that
...
container from `/opt/email-service/.env` into
`/opt/stalwart/etc/config.toml`. If Stalwart is running with stale or incomplete
config after env changes, regenerate it inside the mail LXC:
